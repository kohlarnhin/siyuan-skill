...
一个可通过 GitHub / git 分发的 Claude Code plugin，提供 `/siyuan` skill，直接请求 SiYuan HTTP API。

## 目录结构

- `.claude-plugin/marketplace.json`：让该仓库可作为 marketplace 添加
- `.claude-plugin/plugin.json`：plugin 清单
- `skills/siyuan/`：skill、本地脚本与配置模板
...
- 推荐：使用环境变量
  ...
  - `SIYUAN_TOKEN`
- 备选：复制并填写本地私有配置文件
  - `skills/siyuan/config/siyuan.config.example.json`
  - 到 `skills/siyuan/config/siyuan.config.local.json`
  - 如果你之前用过旧目录结构，请把本地私有配置移动到 `skills/siyuan/config/` 下
...
```text
/siyuan {"action":"get_version","params":{}}
```
