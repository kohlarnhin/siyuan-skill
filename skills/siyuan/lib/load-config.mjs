...
export async function loadConfig({ skillDir }) {
  ...
  const token = normalizeToken(pickFirstDefined(process.env.SIYUAN_TOKEN, localConfig.token, defaults.token));
  ...
  const allowRemote = normalizeBoolean(
    pickFirstDefined(process.env.SIYUAN_ALLOW_REMOTE, localConfig.allowRemote, defaults.allowRemote),
    false,
  );
